...
use crate::read_write::{copy_mle, ReadWriteStream};
use crate::{
    ...
    >,
{
    ...
    fn prove(
        ...
        let (mut h_ps, mut h_qs) = compute_frac_poly_plonk(
            witnesses.clone(),
            pk.permutation_oracles.0.clone(),
            pk.permutation_oracles.1.clone(),
            perm_identity_alpha,
    ...
    fn verify(
        ...
        for i in 0..hp_evals.len() {
            ...
            f_eval +=
                (hp_evals[i] * proof.opening.perm_evals[i] * batch_factor_lower_power * alpha);
            f_eval += (hq_evals[i] * proof.opening.witness_evals[i] * batch_factor_higher_power);
            f_eval += (hq_evals[i]
                * proof.opening.perm_index_evals[i]
