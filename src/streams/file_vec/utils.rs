...
pub(super) fn par_deserialize<T: CanonicalSerialize + CanonicalDeserialize + Sync + Send>(
    ...
    file.take((size * (BUFFER_SIZE - 1)) as u64).read_to_end(work_buffer).ok()?;
    result_buffer.push(val);
    result_buffer.extend(work_buffer.chunks(size).map(|chunk| {
        T::deserialize_uncompressed_unchecked(chunk).unwrap()
    }));
    Some(())
}