...
{
    pub fn commit(input: impl Borrow<[F; INPUT_LEN]>, r: Option<&F>) -> Result<F, RescueError> {
    ...
    pub fn verify(
        input: impl Borrow<[F; INPUT_LEN]>,
        r: Option<&F>,
        comm: &F,
    ) -> Result<VerificationResult, RescueError> {
        if Self::commit(input, r)? == *comm {
            Ok(Ok(()))
        } else {
            Ok(Err(()))
        }
...
#[cfg(test)]
mod test {
    use super::{commitment::FixedLengthRescueCommitment, crhf::RescueCRHF, CRHF_RATE};
