...
impl<F: RescueParameter, const INPUT_LEN: usize, const OUTPUT_LEN: usize>
    FixedLengthRescueCRHF<F, INPUT_LEN, OUTPUT_LEN>
{
    /// ## Padding
    ...
    /// required are added.
    pub fn evaluate(input: impl Borrow<[F; INPUT_LEN]>) -> Result<[F; OUTPUT_LEN], RescueError> {
...
impl<F: RescueParameter, const OUTPUT_LEN: usize> VariableLengthRescueCRHF<F, OUTPUT_LEN> {
    /// ## Padding
    /// if `input` length is not a multiple of `CRHF_RATE`, then it will be
    ...
    fn evaluate<T: Borrow<Vec<F>>>(input: T) -> Result<[F; OUTPUT_LEN], RescueError> {
        let mut output = [F::zero(); OUTPUT_LEN];
